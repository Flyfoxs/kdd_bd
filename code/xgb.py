...
def valid_xgb(train_x, train_y):
    ...
    xgbtrain = xgb.DMatrix(train_x, label=train_y['click_mode'])
    xgbtest = xgb.DMatrix(valid_x, label=valid_y['click_mode'])
    watchlist = [(xgbtrain, 'train'), (xgbtest, 'test')]
    num_rounds = 1000
    model = xgb.train(params, xgbtrain, num_rounds, watchlist, early_stopping_rounds=50,verbose_eval=1)
    val_pred = np.argmax(model.predict_proba(
        valid_x, num_iteration=model.best_iteration), axis=1)
...
def train_xgb(train_x, train_y, test_x):
    ...
    for tr_idx, val_idx in kfold.split(train_x, train_y):
        ...
        xgbtrain = xgb.DMatrix(tr_x, tr_y)
        xgbtest = xgb.DMatrix(val_x, val_y)
        watchlist = [(xgbtrain, 'train'), (xgbtest, 'test')]
        num_rounds = 1000
        model = xgb.train(params, xgbtrain, num_rounds, watchlist,verbose_eval=False, early_stopping_rounds=50)
        val_pred = np.argmax(model.predict_proba(
            val_x, num_iteration=model.best_iteration), axis=1)
...
if __name__ == '__main__':
    ...
    online = 0
    if online == 0:
        valid_xgb(train_x, train_y)
    else:
        result_xgb = train_xgb(train_x, train_y, test_x)
        merge = 0
        if merge == 0:
            submit_result(submit, result_xgb, 'xgb')
        else:
            merge_result(submit,result_xgb, 'xgb')