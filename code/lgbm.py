...
def merge_raw_data():
    tr_queries = pd.read_csv('../data/train_queries_phase2.csv')
    te_queries = pd.read_csv('../data/test_queries.csv')
    tr_plans = pd.read_csv('../data/train_plans_phase2.csv')
    te_plans = pd.read_csv('../data/test_plans.csv')

    tr_click = pd.read_csv('../data/train_clicks_phase2.csv')
...
def valid_lgb(train_x, train_y):
    ...
    train_set = lgb.Dataset(train_x, train_y['click_mode'], categorical_feature=cate_cols)
    val_set = lgb.Dataset(valid_x, valid_y['click_mode'], categorical_feature=cate_cols)
...
def train_lgb(train_x, train_y, test_x):
    ...
    cate_cols = ['max_dist_mode', 'min_dist_mode', 'max_price_mode',
                 'min_price_mode', 'max_eta_mode', 'min_eta_mode', '_first_mode', 'weekday', 'hour']
    scores = []
    result_proba = []
...
if __name__ == '__main__':
    ...
    if online == 0:
        ...
        if merge == 0:
            ...
            merge_result(submit, result_lgb, 'lgb')